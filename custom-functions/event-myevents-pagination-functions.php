...
function myevents_pagination_activate(){
  flush_rewrite_rules();
}

register_activation_hook( __FILE__, 'myevents_pagination_activate');





function myevents_pagination_register_endpoints(){
  register_rest_route(
    'events/v2',
    '/myevents-pagination/',
    array(
      'methods' => 'POST',
      'callback' => 'get_more_results_myevents',
         'args' => array(
            'offset' => array(
                'required'          => true,
	  	        'type'              => 'integer',
...
add_action( 'rest_api_init', 'myevents_pagination_register_endpoints');

/* -----------------------------------------------------------REQUEST CALLBACK-----------------------------*/
function get_more_results_myevents(WP_REST_Request $request){			
/*------------------------------------------------------------------HTML-RESPONSE------------------------------*/	
